function [mean_velocity_all, SD_velocity_all, all_velocity] = BIPN145_flytrack2(diameter,frame_rate,varargin)
...
for i = 1:num_files
    ...
    mean_velocity = mean(all_velocity(i,:),'omitnan');
    std_velocity = std(all_velocity(i,:),'omitnan');
...
%% FINAL DATA SUMMARY FOR ALL FILEs
...
disp('Standard deviation (NOT error) of mean velocity ACROSS videos is:')
    if num_files == 1
    SD_velocity_all = std(all_velocity,'omitnan');
    else
